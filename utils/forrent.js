...
async function forrent(
  ...
  try {
    ...
    await gotScraping
      .get({
        url: `https://www.funda.nl/en/${searchLocation}/${priceMin}-${priceMax}/${minLivingArea}+woonopp/${propertyType}/${minBedroom}+slaapkamers/`,
        ...
        .then(({ body }) => {
          ...
          result.each(async (idx, el) => {
            ...
            if (isExists) {
              ...
              await upsertPropertyForRent(client, uuid, {
                _id: uuid,
                name: name,
                location: location,
                price: price,
              ...
              // create Notion entry
              var notionID = await notion.createRent(
                (uuid = uuid),
                (name = name),
                (location = location),
                (price = price),
...
async function upsertPropertyForRent(client, uuid, property) {
...
async function findProperty(client, uuid) {
  return await client.db("funda").collection("forrent").findOne({ _id: uuid });
}
